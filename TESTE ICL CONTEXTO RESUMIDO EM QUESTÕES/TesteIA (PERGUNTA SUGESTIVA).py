...
resultado = ollama.generate(model=MODELO, prompt="Bom dia")
...
contexto = open('regulamento_resumido.txt', encoding='utf-8').readlines()
...
while True:
    ...
    if pergunta_original == 0:
        pergunta_original = input("Digite a pergunta: ")
    else:
        pergunta_original = perguntas[pergunta_original-1]

    # Cria o prompt completo para o modelo, combinando contexto e pergunta
    pergunta = f"Considere o contexto abaixo e responda resumidamente em português a pergunta a seguir:\n\nContexto:\n{contexto}\n\nPergunta: {pergunta_original}"
    ...
    with open('Saida_ICL_Resumido.txt', 'a', encoding="utf-8") as arquivo:
